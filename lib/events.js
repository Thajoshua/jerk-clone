...
function Index(info, func) {
    ...
    const infos = {
        ...
        vpsOnly: info.vpsOnly ?? false,
        function: async (message, match, client) => {
            if (infos.vpsOnly && !config.VPS) {
                return await message.reply('This command is only available on VPS');
            }
            return await func(message, match, client);
        }
    };
