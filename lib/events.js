...
function Index(info, func) {
	...
	if (!info.on && !info.pattern) {
		...
		if (info.on) {
			infos.on = info.on;
			if (info.pattern) {
				infos.pattern = new RegExp((info.handler ?? true ? PREFIX : '') + `\\b${info.pattern}\\b`, info.flags ?? '');
			}
		} else {
			infos.pattern = new RegExp((PREFIX.startsWith('^') ? PREFIX : '^.+' + PREFIX) + `\\b(${info.pattern})\\b`, 'is');
		}
	}
