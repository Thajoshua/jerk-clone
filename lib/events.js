...
function Index(info, func) {
	...
	if (!info.on && !info.pattern) {
		...
		if (info.on) {
			infos.on = info.on;
			if (info.pattern) {
				// Add anchors to match exact patterns
				infos.pattern = new RegExp(
					`^${info.handler ?? true ? PREFIX : ''}${info.pattern}$`, // Ensure strict match
					info.flags ?? ''
				);
			}
		} else {
			infos.pattern = new RegExp(`${config.HANDLERS}(${info.pattern}(?:\\s+\\S+)*)`, 'is');
		}		
	}
